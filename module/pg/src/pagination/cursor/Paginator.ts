...
import { atob, btoa, decodeByType, encodeByType, pascalToUnderscore } from './utils';
...
export default class Paginator<Entity extends ObjectLiteral> {
    ...
    private buildOrder(): OrderByCondition {
        ...
        this.paginationKeys.forEach((key) => {
            const convertKey = key.replace(/[A-Z]/g, (v) => `_${v}`).toLowerCase();
            orderByCondition[`${this.alias}.${convertKey}`] = order;
        });
